...
test('All five Spot the Difference sets have five paired normalized hotspots',()=>{const ids=[...spot.matchAll(/id:'approved-\d+'/g)];assert.equal(ids.length,5);const pairs=[...spot.matchAll(/top:\[([.\d]+),([.\d]+)\],bottom:\[([.\d]+),([.\d]+)\]/g)];assert.equal(pairs.length,25);for(const m of pairs){const [tx,ty,bx,by]=m.slice(1).map(Number);assert([tx,ty,bx,by].every(v=>v>=0&&v<=1));assert(ty<.5&&by>.5)}});
